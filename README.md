...
ホーム画面に追加すると、Android では YouTube の共有先に「おうちトレ」が出るようになり、動画から直接メニューを作れます。
...
- 使うには、**あなた自身の Gemini の API キー**が要ります（Google AI Studio で無料で作れます。無料枠があります）。設定の「動画を読むキー」に貼ります。
...
このアプリが外に出す通信は、動画のサムネイル（`i.ytimg.com`）、URL から題名を取る YouTube oEmbed、そしてキーを入れたときだけの Gemini、の3つです。
