...
<img src="images/01_welcome.png" width="300" alt="1 / 3 トレ録へようこそ"> <img src="images/02_nickname.png" width="300" alt="2 / 3 何とお呼びすればいいでしょうか"> <img src="images/03_companion.png" width="300" alt="3 / 3 相棒をひとつ">

三つ答えると、ホーム画面に追加の案内が出て、そのあと最初の画面（ホーム）になります。

<img src="images/05_home_empty.png" width="300" alt="まだ記録が無いホーム">

ホームには、今週の実績（曜日ごとの四角）、きょう動いた分、今日の記録、いちばん下に **記録する** のボタンが並びます（その日に記録があると **もう1つ記録する** に変わります）。ホームは見るための画面で、記録は **記録する** から行います。
...
こうして残した記録は **セルフトレ** という名前で並びます（動画に紐づかない記録の絵が付きます）。セルフトレは1日に1つです。同じ日にもう一度開くと、さきほどの種目が入った状態で開き、足した分が加わります。

---

## 5. 過去の日を直す

ホームの **前の日の記録を見る** を押すと、月のカレンダーが開きます（**今週の実績** の四角を押しても、その日が選ばれた状態で開きます）。記録のあった日は四角が塗られ、記録が多い日ほど濃くなります。日を押すと、その日の記録が下に出ます。記録の行を押すと **記録を修正** の画面になります。記録が無い日を押すと **この日に記録する** が出て、その日付で記録できます。

<img src="images/17_history.png" width="300" alt="前の日の記録（月のカレンダー）の画面"> <img src="images/18_edit_record.png" width="300" alt="記録を修正の画面">
...
今日の記録は、ホームの **今日の記録** の行を押しても同じ画面が開きます。
...
**設定 → 名前** でニックネームを変えられます。ニックネームは、その日はじめて出る一言の頭で一度だけ呼びます。同じ日のうちは、そのあと呼びません。

ホームの上には、相棒からの一言が出ます。時間帯・曜日・季節や、前回やった時間などに合わせて、ちょっとした声をかけてきます（例：「こんにちは。もうお昼過ぎましたね。ご飯食べて眠くなってませんか？」）。朝・昼・夜など時間帯が変わると言うことも変わります。問いかけには答えなくて構いません。**褒めたり、急かしたり、続けた日数を数えたりはしません。**天気などを外へ取りに行くこともありません。

---

## 9. 見た目を変える

**設定 → 見た目** で、アプリ全体の色を三つから選べます。文字の大きさや並び方は変わりません。

| 名前 | 見た目 |
|---|---|
| 紙（既定） | 生成りの地・クリーム色のカード |
| いまのトレ録 | 青みの灰色の地・白いカード |
| 夜 | 暗い地・明るい文字 |

選択中は、丸の中の ✓ と太い枠と「選択中」の札で分かります。選んだ見た目はスマホに保存され、次に開いたときもそのままです。
...
**注意と免責。** トレ録は記録帳で、運動のやり方や量を勧めるものではありません。体調に合わせて、痛みや不調があれば無理をしないでください。記録はあなたの端末の中にだけあり、ブラウザの都合で消えることがあります。消えた記録はもとに戻せないので、控えは「設定 → 記録を残す」の書き出しで残してください。このアプリは無料で現状のまま提供しており、動作や記録の保持は保証できません。利用によって生じた損害について作者は責任を負いません。同じ内容は「設定 → このアプリについて」にもあります。
